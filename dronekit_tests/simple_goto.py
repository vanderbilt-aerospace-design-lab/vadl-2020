...
# -*- coding: utf-8 -*-
""" © Copyright 2015-2016, 3D Robotics. simple_goto.py: GUIDED mode "simple goto" example (Copter Only) Demonstrates how to arm and takeoff in Copter and how to navigate to points 
...
#parser = argparse.ArgumentParser(description='Control Copter and send commands in GUIDED mode ') parser.add_argument('--sitl',
#                   help="Vehicle connection target string. If specified, SITL will be used.") parser.add_argument('--indoors',
 #                  help="Vehicle connection target string. If specified, SITL will be used.") args = parser.parse_args() sitl = None
...
#vehicle.parameters['ARMING_CHECK'] = 0 if args.indoors:
...
def decorated_mode_callback(self, attr_name, value):
    # `attr_name` is the observed attribute (used if callback is used for multiple attributes) `attr_name` - the observed attribute (used if callback is used for multiple 
    # attributes) `value` is the updated attribute value.
